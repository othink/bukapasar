...
class Produk extends CI_Controller {
	...
	function sopir_get_cost(){
		...
		foreach ($sopir->result_array() as $row) {
			...
			$sopir = $this->db->query("SELECT a.*, b.kecamatan_id FROM rb_sopir a JOIN rb_konsumen b ON a.id_konsumen=b.id_konsumen where a.aktif='Y'");
	...
	function selesai_belanja(){
		if (isset($_POST['submit'])){
	        ...
	        $message  = "<html><body><img src='".base_url()."asset/logo/$logo[gambar]' style='height:87px'><br>
				...
				$message  .= "<br>Untuk melihat Rincian pemesananmu bisa melalui halaman berikut <a style='text-decoration:none; color:green' target='_BLANK' href='".base_url()."konfirmasi/tracking/$kode_transaksi'>".base_url()."konfirmasi/tracking/$kode_transaksi</a>, <br><br>
...
//$isi_pesan .= "Butuh Metode Pembayaran Lainnya? klik disini : ".base_url()."konfirmasi/bayar?inv=$kode_transaksi";
...
Hai *$kons[nama_lengkap]*, Terima kasih telah berbelanja di $iden[url], pembayaran anda untuk orderan ini telah kami terima Dan kami telah menginformasikan kepada seller untuk segera diproses.";
	...
	function checkouts(){
		...
		if (isset($_POST['submit'])){
			...
			//}else{
				//$this->template->load(template().'/template',template().'/reseller/view_checkouts_options',$data);
