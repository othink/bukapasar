...
class Auth extends CI_Controller {
		...
		if (isset($_POST['submit2'])){
			if (cetak($this->input->post('password'))==cetak($this->input->post('repassword'))){
				...
				if ($cek_username->num_rows()<='0'){
					...
					$isi_pesan = "Hai *Kak ".$this->input->post('username')."* Terima kasih telah mendaftar di *$iden[url]* Silakan untuk melengkapi data diri anda sesuai dengan identitas pada KTP di ".base_url()."members/edit_profile username: *".$this->input->post('username')."* password: *".$this->input->post('password')."* Siap mencari produk dari ribuan penjual online di Indonesia? ".base_url()."produk";
					...
					if($kons['email'] !=""){
						...
						echo $this->session->set_flashdata('message', '<div class="alert alert-success"><center>Terima kasih, anda sudah terdaftar di <b>'.$iden['url'].'</b> silahkan cek WA verifikasi!</center></div>');
						$this->model_app->wa(format_telpon($kons['no_hp']),$isi_pesan);
					}
	...
	public function lupass(){
		if (isset($_POST['submit3'])){
			...
			$cek = $this->db->query("SELECT * FROM rb_konsumen where email='$email' OR username='$email' OR no_hp LIKE '%$no_telpon'");
		    $total = $cek->num_rows();
			if ($total > 0){
				$data = $cek->row_array();
				$tgl_kirim = date("d-m-Y H:i:s");
				...
				$isi_pesan = "Hai *Kak ".$data['nama_lengkap']."* Hari ini pada tanggal ".$tgl_kirim." anda meminta akses untuk reset password... Silahkan klik link berikut untuk mengubah password : ".base_url()."auth/resetpassword?token=$data[password].$data[id_konsumen] Jika permintaan ini bukan dari anda maka silahkan melaporkannya kepada kami dengan mengirim email ke $iden[email] atau balas WA ini. *Terima Kasih*";
				$this->model_app->wa(format_telpon($data['no_hp']),$isi_pesan);
			}else{
