...
class Main extends CI_Controller {
	public function email(){
		//echo kirim_email('Coba Kirimkan Email','Ini adalah isi Pesan yang dikirim...','edi.suwoto@gmail.com');
		//echo "E-mail Terkirim,..";
		if(reseller($this->session->id_konsumen) == ""){
				echo $this->session->set_flashdata('message', '<div class="alert alert-danger"><center>Maaf, Anda harus menjadi Reseller Terlebih dahulu,.. <a href="'.base_url().'members/buat_toko"> Daftar Reseller</a></center></div>');
				redirect('main');
			}else{

				echo "edi ".reseller($this->session->id_konsumen);
				echo "<br>".saldo(reseller($this->session->id_konsumen),$this->session->id_konsumen);
			}
	...
	function proses(){
		...
		if (isset($_POST['submit'])){
			...
			$harga = $harga + 150;
					//echo reseller($this->session->id_konsumen)." edi";
			$saldo = saldo(reseller($this->session->id_konsumen), $this->session->id_konsumen);
			if ($saldo >= $harga){
