...
class Members extends CI_Controller {
	...
	function koperasi(){
		...
		$data['row'] = $this->model_reseller->profile_konsumen($this->session->id_konsumen)->row_array();
		$this->template->load(template().'/template',template().'/reseller/view_koperasi',$data);
	}
	function daftar_koperasi(){
		...
		if (isset($_POST['submit'])){
			...
			$data = array('id_konsumen'=>$this->session->id_konsumen,
							'nomor_ktp'=>cetak($this->input->post('nomor_ktp')),
							'lainnya'=>cetak($this->input->post('lainnya')),
							'lampiran'=>$fileName);
			$cek_kop = $this->model_app->view_where('rb_koperasi',array('id_konsumen'=>$this->session->id_konsumen));
			if ($cek_kop->num_rows()>=1){
				$where = array('id_konsumen' => $this->session->id_konsumen);
				$this->model_app->update('rb_koperasi', $data, $where);
			}else{
				$this->model_app->insert('rb_koperasi',$data);
			}
			redirect('members/koperasi');
		}else{
			$data['title'] = 'Lengkapi Data Koperasi';
			$data['row'] = $this->model_reseller->profile_konsumen($this->session->id_konsumen)->row_array();
			...
			$data['rows'] = $this->db->query("SELECT a.*, b.nama_lengkap, b.no_hp, b.kecamatan_id, b.kota_id FROM rb_koperasi a JOIN rb_konsumen b ON a.id_konsumen=b.id_konsumen 
                                                          where a.id_konsumen='".$this->session->id_konsumen."'")->row_array();
			$this->template->load(template().'/template',template().'/reseller/view_koperasi_edit',$data);
		}
	}
	...
	function trx_pulsa(){
		cek_session_members();
		$jumlah= $this->db->query("SELECT * FROM `rb_pembelian_pulsa` where (id_reseller='".reseller($this->session->id_konsumen)."' OR id_konsumen='".$this->session->id_konsumen."') ORDER BY waktu_pembelian DESC")->num_rows();
		...
		$data['row'] = $this->model_reseller->profile_konsumen($this->session->id_konsumen)->row_array();
		$data['pulsa'] = $this->db->query("SELECT * FROM `rb_pembelian_pulsa` where (id_reseller='".reseller($this->session->id_konsumen)."' OR id_konsumen='".$this->session->id_konsumen."') ORDER BY waktu_pembelian DESC LIMIT $dari,$config[per_page]");
		$this->pagination->initialize($config);
	...
	function print_kirim(){
		cek_session_members();
		verifikasi(reseller($this->session->id_konsumen));
		$data['title'] = "Detail Data Penjualan";
		$data['rows'] = $this->model_reseller->penjualan_konsumen_detail_reseller($this->uri->segment(3))->row_array();
		$data['record'] = $this->model_app->view_join_where('rb_penjualan_detail','rb_produk','id_produk',array('id_penjualan'=>$this->uri->segment(3)),'id_penjualan_detail','ASC');
		//$this->template->load(template().'/template',template().'/reseller/mod_penjualan/view_penjualan_detail_print',$data);
		
		$this->load->view('cetak/kirim',$data);
	}
